...
class StdAverageMeter(object):
    """Computes average, current value, and standard deviation."""
...
class FoldEpochMat:
    ...
    def result(self, fold):
        fold += 1
        criterion = sum(self.key_to_mat_dict[key][:fold].mean(axis=0)
                        for key in self.best_result_keys)
        best_epoch = int(criterion.argmax())

        mean_ret = {}
        std_ret = {}
        for key, mat in self.key_to_mat_dict.items():
            column = mat[:fold, best_epoch]
            mean_ret[key] = column.mean()
            std_ret[key] = column.std()

        return mean_ret, std_ret, best_epoch + 1

    def update_best_epoch(self, fold):
        criterion = sum(self.key_to_mat_dict[key][fold:fold + 1]
                        for key in self.best_result_keys)
        best_epoch = int(criterion.squeeze().argmax())

        ret = {'fold': fold, 'best_epoch': best_epoch}
        for key, mat in self.key_to_mat_dict.items():
            ret[key] = mat[fold, best_epoch]

        self.best_epoch_of_fold[f'fold_{fold}'] = ret
    ...
    def average_all_best_epoch_of_fold(self):
        for k in self.keys:
            tmp = sum(values[k] for _, values in self.best_epoch_of_fold.items())
            self.average_all_best_epoch[k] = round(tmp / self.num_fold, 4)
