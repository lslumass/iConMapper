...
def arg_parse():
    parser = argparse.ArgumentParser(
        'Deep Supervised Graph Partitioning Model with Type Prediction Enhancement (DSGPM-TP)')
    ...
    parser.add_argument('--tb_root', type=str, default='./tb_logs',
                        help='Tensorboard log directory')
    parser.add_argument('--ckpt', type=str)
    parser.add_argument('--devices', type=str, default='0')
    parser.add_argument('--lr', dest='lr', type=float, help='Learning rate.')
    parser.add_argument('--batch_size', dest='batch_size', type=int, help='Batch size.')
    parser.add_argument('--epoch', dest='epoch', type=int, help='Number of epochs to train.')
    ...
    parser.add_argument('--no_use_regular_mapping_from_prediction', dest='use_regular_mapping_from_prediction',
    ...
    parser.add_argument('--inference_method', choices=['dsgpm_tp', 'spec_cluster', 'metis', 'graclus'],
                        default='dsgpm_tp')
    ...
    # Set input_dim from ATOMS dictionary
    args.input_dim = len(ATOMS)

    # Parse devices string to list of ints
    args.devices = [int(d) for d in args.devices.split(',')]

    # Set device for affinity matrix
    args.device_for_affinity_matrix = torch.device(args.device_for_affinity_matrix)

    # Validate tb_root when tb_log is enabled
    if args.tb_log and args.tb_root is None:
        raise ValueError("--tb_root must be specified when using --tb_log")
